...
std::vector<std::vector<int>> ReadFile(std::string fileName) {
	...
	while (getline(inFile, line)) {
		...
		int num;
		while (sstream >> num) { //convert each value read to an integer
			row.push_back(num);  
		}
...
int main(int argc, char* argv[]) {
	...
	std::cout<<"Matrix 1"<<std::endl; //print first matrix file 
	PrintMatrix(matrix1);
	std::cout << std::endl;
	
	std::cout<<"Matrix2"<<std::endl; //print second matrix file
	PrintMatrix(matrix2);
	std::cout << std::endl;
	...
	makeList(&matrix1, &matrix2, m1, m2); //make linked list of out the two vectors obtained from the input file. 

	LinkedMatrix* m3 = new LinkedMatrix(); //output linked list containing the sum of two lists 
	LinkedMatrix* m4 = new LinkedMatrix(); //output linked list containing the product of m1 and m2 linked lists

	m3->addMatrix(m1->head, m2->head, m3);
	m4->multiplyMatrix(m1->head, m2->head, m4);
